...
- `src/index.css` — Tailwind v4 entry: font imports (@fontsource, self-hosted), `@theme` design tokens, the `html[data-theme='light']` token override block, Arabic letter-spacing guard, `.ph-media` gradient, the single marquee keyframes. **Accent rule:** `--color-accent` (caramel #C08552) is for FILLS only; accent-colored TEXT must use `--color-accent-ink` (caramel on dark, coffee #6F4E37 on light).
